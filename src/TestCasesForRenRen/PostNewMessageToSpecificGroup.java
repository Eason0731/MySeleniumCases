...
public class PostNewMessageToSpecificGroup {
  ...
  public void PostNewMessageTesting() throws Exception {
	  ...
	  //否则因为时间过快，也不会被激活
	  MyFenzuList(); //选择普通朋友作为新鲜事发布可见的分组
	  Thread.sleep(3000);
  ...
  private void SetBackSpaceFunction() //该方法用于模拟键盘操作，这里模拟键盘上按下和释放BACKSPACE键的操作
  {
  	...
  	public static void MyFenzuList() throws Exception{
	  driver.findElement(By.className("ui-select-selected")).click(); //先找到分组框，并点击一下
	  Thread.sleep(5000);
	  //driver.findElement(By.xpath("//*[@id='ui-id-22']")).click(); //展开并找到“显示更多分组”，再次点击一下
	  //Thread.sleep(5000);
	  
	  //新建数组FenzuList并存储搜索框存储的下拉列表框的内容
	  List<WebElement> FenzuList = driver.findElements(By.xpath("//*[@id='nxContainer']/div/div[5]/div[2]/div/div[1]/div/div/div[3]/div[2]/div[2]"));
	  Thread.sleep(5000);
	  for (WebElement mygroup: FenzuList)
	  {
		  if (mygroup.getText().contains("普通朋友")) //选择普通朋友作为可见的分组
		  {
			  mygroup.click();
		  ...
		  System.out.println(mygroup.getText());
